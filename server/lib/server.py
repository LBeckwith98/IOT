...
class Server:
    ...
    def __init__(self):
        ...
        self.connDict = {
            "ALARM_CLOCK": {
                "active": False,
                "conn": None,
                "selfAddr": "192.168.2.3",
                "address": "192.168.2.4", #"192.168.2.4",
                "port": 12345,
                "socket": None
            }
        }
        for connection in self.connDict:
            ...
            self.connDict[connection]["socket"].setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    ...
    def refreshConnection(self):
        for connection in self.connDict:
            if self.connDict[connection]["active"] is False:
                self.connDict[connection]["socket"].listen(5)
                print('listened')
                ...
                if ready[0]:
                    ...
                    print("actived, see = " + str(self.connDict["ALARM_CLOCK"]["active"]))
                    self.numConns += 1
    ...
    def sendCmdTo(self, connName, cmd):
        try:
            if self.connDict[connName]["active"] == False:
                print(connName + ' active = '+ str(self.connDict[connName]["active"]))
                return False
            data = pickle.dumps(cmd, protocol=2)
...
# server = Server()
# while server.refreshConnection() == 0:
#     print('pass')
#     time.sleep(1)